...
class Validator {
  ...
  constructor(schema, options) {
    ...
    this.config = {
      root: options.root || options.name || 'root',
    ...
    if (options.name)
      schemas[options.name.toLowerCase()] = this;
  }
  ...
  getPropsByName(name) {
    ...
    for (let prop in this.properties) {
      if (this.properties[prop][name] !== undefined)
        results.push({
          field: prop,
  ...
  test(src, throwErr = false) {
    ...
    if (src === undefined) {
      ...
      if (this.config.throwMode || throwErr) {
