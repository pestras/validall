  ...
  if (Array.isArray(type)) {
    if (Array.isArray(value)) {
      if (!V.negateMode) {
        ...
        if (!match)
          throw new ValidallError('$type', `${path}[${index}]`, message || `${path}[${index}] must be of type ${type}`, `${Types.getTypesOf(value[index])}: ${value[index]}`);
      } else {
    ...
    if (typeof type === "string") {
      ...
      if (schemas.hasOwnProperty(schemaName.toLowerCase())) {
        if (arrayType && Array.isArray(value))
          ...
          throw new ValidallError('$type', path, message || `${path} must be of type ${type}`, `${Types.getTypesOf(value)}: ${value}`);
        }
      }
    }
    ...
    next.call(V, Types.getTypesOf(value), type, path + '.' + '$type');
  }
}
