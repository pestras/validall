...
function $type(value, type, path, message, V) {
  if (Types.function(type)) {
    if (V.negateMode && (value instanceof type))
      throw new ValidallError('$type', path, message || `"${path}" must not be instance of "${type}"`, value);
    else if (!V.negateMode && !(value instanceof type))
      throw new ValidallError('$type', path, message || `"${path}" must be instance of "${type}"`, value);
    ...
    let match = false;
    if (valueType.indexOf('[]') > -1 && type === 'array')
      match = true;
    else if (type === 'any')
      match = true;
    else
...
function $all(value, list, path, message, V) {
  ...
  for (let i = 0; i < value.length; i++)
    if (list.indexOf(value[i]) === -1)
      if (!V.negateMode)
        throw new ValidallError('$all', path, message || `"${path}" must be all included in [${list}]`, value);

  if (V.negateMode)
    throw new ValidallError('$all', path, message || `"${path}" must not be all included in [${list}]`, value);
}
